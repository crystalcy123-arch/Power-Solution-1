...
import { ADUConfig, UserLocation } from '../types';
...
const ADUSection: React.FC<ADUSectionProps> = ({ location }) => {
  ...
  const activeRegion = useMemo(() => {
    return (location.region && location.region in regionalKnowledge) ? location.region : 'ON';
  }, [location.region]);
  ...
  const handleBookSession = async () => {
    ...
    try {
      // 提交逻辑：加入 _gotcha 字段防止 Spam
      const response = await fetch("https://formspree.io/f/xreadzbr", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          _subject: `New ADU Design Inquiry: ${contact.name}`,
          _gotcha: "", // 核心修复：防止被标记为垃圾邮件
          ...
          location: `${location.city || 'St. Catharines'}, ${location.region || 'ON'}`
        })
      });
