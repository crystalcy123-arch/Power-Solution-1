...
const SolarSection: React.FC<SolarSectionProps> = ({ location }) => {
  ...
  const techFAQs = [
    ...
    { q: "Does this equipment export power back to the grid? Will it mess with our utility transformer?", a: "No. Our systems are strictly designed for Behind-The-Meter Load Displacement. We install a reverse-power relay that guarantees no power ever flows backward into the utility grid. Connection approvals with local utilities like NPEI and Alectra are fast and straightforward." },
    { q: "How much physical space do you need?", a: "Very little. A standard commercial 500 kW / 1000 kWh battery system is self-contained in a weatherproof enclosure roughly the size of a 20-foot shipping container." },
    { q: "Who handles maintenance and repairs?", a: "We do, 100%. Because we own the asset, we actively monitor the battery's health 24/7. We dispatch our own technicians at our own cost." }
  ];
  ...
  const financialFAQs = [
    ...
    { q: "Will this system go on our balance sheet as a capital lease or debt?", a: "No. This is structured as Energy-as-a-Service (EaaS). Because we retain full legal ownership, it remains entirely off your balance sheet." },
    { q: "What if the battery degrades over 10 years and stops saving us money?", a: "We carry all the performance risk. Our revenues are directly tied to the system’s performance. If the battery degrades, we lose money." },
    { q: "Does this affect our property or liability insurance?", a: "No. We carry comprehensive commercial general liability and property insurance specifically for our energy assets." }
  ];
    ...
    <section className="max-w-7xl mx-auto px-4 py-12 space-y-32">
      ...
      <div className="grid lg:grid-cols-2 gap-16 items-start">
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-3 gap-3">
                      {['industrial', 'office', 'retail', 'multi-unit', 'farm', 'others'].map(t => (
                        <button key={t} onClick={() => setComNeeds({...comNeeds, facilityType: t as any})} className={`p-4 rounded-xl border-2 font-bold capitalize transition-all text-sm ${comNeeds.facilityType === t ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-slate-50 border-slate-50 text-slate-600'}`}>{t.replace('-', ' ')}</button>
                      ))}
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <textarea placeholder="Custom requirements..." value={comNeeds.notes} onChange={e => setComNeeds({...comNeeds, notes: e.target.value})} className="w-full p-5 rounded-xl border-2 border-slate-100 font-medium bg-slate-50 min-h-[100px] resize-none focus:border-emerald-500 outline-none transition-colors" />
                  </div>
                )}
        ...
        <div className="relative lg:sticky lg:top-24">
          <div className="h-full rounded-[3.5rem] overflow-hidden border-4 border-white bg-white flex flex-col items-center justify-center min-h-[500px] shadow-2xl relative">
